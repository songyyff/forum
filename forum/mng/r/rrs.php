...
<table width="100%" class=bdb1 border=1 cellpadding=0 cellspacing=0>
	...
	<td width="160">&nbsp;</td>
	<td colspan="2">
	<input type=radio id=stri name=stype onclick="ckstype(this)" value=0<?php if($_R['stype']==0)echo $c;?>>回复编号 <input type=radio id=stii name=stype onclick="ckstype(this)" value=3<?php if($_R['stype']==3)echo $c;?>>帖子编号 <input type=radio id=stui name=stype onclick="ckstype(this)" value=1<?php if($_R['stype']==1)echo $c;?>>用户编号 <input type=radio id=stu name=stype onclick="ckstype(this)" value=2<?php if($_R['stype']==2)echo $c;?>>用户名 <input type=radio id=stup name=stype onclick="ckstype(this)" value=4<?php if($_R['stype']==4)echo $c;?>>用户名片段</td>
	</tr>
	<tr>
	<td>&nbsp; &nbsp; 查询标记</td>
...
<table width="100%" class="llbgc" border=1 cellpadding=0 cellspacing=0>
	...
	<td width=160 style="padding-top:1px"><input type=text id=serstr name=serstr maxlength=200 value="<?php echo f_rpspc($_R['serstr']);?>" onblur="checkserstr(this)"></td>
	<td>填写您想查询的单词</td>
	</tr>
...
<tr height=23>
	<TD width=160 rowspan=2>&nbsp;<input type=checkbox id=ext3 name=isright onclick="ckrigt(this)" value=1<?php echo $_R['isright']?$c:"";?>>权限<input type=hidden id=right name=right value=<?php echo $_R['right']?f_rpspc($_R['right']):"11111"; ?>></td>
...
<SCRIPT language="JavaScript"><?php echo"boxs=[$boxs]";?></script>
<SCRIPT language="JavaScript" src="../js/mng/rss.js"></script>
...
echo"]
for(i=0;i<$len;i++) G('sbox'+subboxs[i]).checked=true
";
}
