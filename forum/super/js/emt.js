...
function start(){
s="<table width=100% cellspacing=0 cellpadding=5><tr class=bar2><td width=130>表情类别<td>名称<td width=100>拥有表情数量";
for(i=0;i<ts.length;i++)s+="<tr onclick=fillalt(this)"+(i&1?" class=tr":"")+"><td>"+ts[i][0]+"<td>"+ts[i][1]+"<td>"+ts[i][3]
...
v=ts[--v]
G('tname').parentNode.innerHTML="<input type=text name=tname id=tname value=\""+v[1]+"\">"
}
}
...
break;
case '2':
s+="类别编号</td><td width=30><input type=text id=tno name=tno class=input30></td><td align=right width=60>类别名称<td><input type=text id=tname name=tname>";
break;
case '3':
s+="类别编号</td><td><textarea id=delts name=delts rows=2 cols=60></textarea></td></tr><tr><td></td><td>格式：类别号 [ ,类别号 ] ...<br>比如要删除12、18和20号类别，只要填入 12,18,20 就行了, 只有类别拥有表情数量是0时才可删除";
break;
case '4':
s+="类别名称</td><td><input type=text name=tname id=tname>";
break;
}
return s+="</table>";
}
