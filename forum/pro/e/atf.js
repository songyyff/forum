...
function openbox(t){var d=t.d;with(G('debug').style){visibility=d?"hidden":"visible";position=d?"absolute":"static"}D.style.height=d?370:200;t.d=!d}

var fU=G('func'),fI=fU.lastChild,fS=fU.firstChild,
...
G('doview').onclick=function(){okV=!okV;this.innerHTML=okV?"取消":"预览"}
...
function vIload(){vI.onload=null
...
function pin(){
var t=this,d=t.d
with(fU.style){height=22;width=79;top=t.offsetTop-D.scrollTop+6}
hold()
//alert(curI)
//if(curI!=t){
if(curI)curI.style.borderColor=""
if(okV)if(!d[1]&&F[6*d[0]+3].substr(0,2)=="i/"){
a=W.style
b=V.style
b.top=a.top
...
curI=t
//}
t.style.borderColor=Sc;fileDisabled=d[2]
fI.setTxt(d[1]||d[4]?"取消":"替换")
fS.disabled=d[1];fS.value=d[2]?"取消":"删除"
if(t.lastChild.d&&!d[4]){if(d[0]>=0&&d[3]==0)t.className="";t.removeChild(t.lastChild);t.removeChild(t.childNodes[1])}
}
function pout(){timeID=setTimeout(outi,500)}

function outi(){with(fU.style)height=width=0
curI.style.borderColor=""
fileDisabled=fS.disabled=1
V.style.top=-1000}

for(i=0,n=F.length/6;i<n;i++)if(F[p=i*6]){Num++
A(D,t=C('p'))
t.innerHTML="<b>"+formatNum(F[p+4])+" B</b><u onmouseover=hold() onclick=r()>"+lt(F[p+1])+"</u><span>"+lt(F[p]+" "+F[p+5])
t.d=[i,0]

function oy(o){for(var y=0;o;y+=o.offsetTop,o=o.offsetParent);return y}
t.onmouseout=pout
t.onmouseover=pin
}
...
function formatNum(i){var r,s=i.toString();i=s.length-3;r=s.substr(i,3);for(;i-3>0;i-=3)r=s.substr(i-3,3)+","+r;return (i>0?s.substr(0,i)+",":"")+r}
...
if(d[1]){
rpI.innerHTML="<b>"+formatNum(F[(p=d[0]*6)+4])+" B</b><u onmouseover=hold() onclick=r()>"+lt(F[p+1])+"</u><span>"+lt(F[p]+" "+F[p+5])
rpI.className=""
d[1]=0
fI.setTxt("替换")
...
t.innerHTML="<b>"+formatNum(s)+" B</b>"+lt(n)+"<span>"+(m?parseInt(F[t.d[0]*6]):"-1")+" "+lt(d)
t.className="uprow"
t.lastChild.d=0
return 1
}
...
t=C('div')
t.className="upper"
r.insertBefore(t,r.childNodes[1])
r.d[4]=1
...
n.childNodes[1].style.width=Math.ceil((p=d/a)*(n.offsetWidth-2))
n.lastChild.innerHTML=Math.ceil(p*100)+"%"
}

var Es=["HTTP","IO","Security"]
function upfileError(i,m,t,s){
var p,n=(m?rpfiles:upfiles)[i];
n.lastChild.innerHTML+=" upfile "+Es[t]+" Error:"+lt(s)
n.style.backgroundColor=Ec
n.d[4]=0
DB("上传文件 "+lt(n.childNodes[2].data)+" <u>失败</u>。传送进度 "+lt(n.lastChild.innerHTML))
//n.lastChild.d=1
}

function upfileComplete(i,m){
var n=m?rpfiles[i]:upfiles[i]
n.childNodes[1].style.width=n.offsetWidth-2
...
//alert(c.charCodeAt(0).toString(16))
if(c>'0'&&c<':'){
d=d.split(".")
a.lastChild.d=1
a.d[4]=a.d[1]=a.d[5]=0
DB("上传文件 "+lt(a.childNodes[2].data)+" 成功")
if(m){
a.d[1]=0
c=a.d[3]
p=a.d[0]*6
...
delete c
a.replaceChild(t=C('u'),a.childNodes[2])
t.innerHTML=lt(F[p+1])
t.onmouseover=hold
t.onclick=r
a.childNodes[3].innerHTML=lt(F[p]+" "+d[0])
}else{
DB("上传文件: "+lt(a.childNodes[2].data)+" <U>失败</U>. "+lt(d))
if(m)a.d[4]=a.d[5]=0
//a.lastChild.innerHTML+=" 上传失败: "+lt(d)
}
}
...
function Cancel(){with(W.style)left=top=-999}
