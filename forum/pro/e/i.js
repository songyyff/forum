...
D=document
A(D.getElementsByTagName("head")[0],s=C('link'))
s.type="text/css"
s.rel="stylesheet"
s.href="../theme/"+tTheme+"/edit.css"

iF.innerHTML="<a name=replaysite></a><div class=hb1p1>发表"+(vT?"回复":"帖子")+"</div><FORM id=MF method=POST><div style=padding:20px><input name=til id=til style=width:100%><input type=hidden name=Icode id=Icode><div class=p5 id=mIcons></div><OBJECT ID=dlgHelper CLASSID='clsid:3050f819-98b5-11cf-bb82-00aa00bdce0b' style='width:0;height:0'></OBJECT><div id=EW contenteditable></div><div id=Info>状态</div><div id=Aview><img></div><input type=button onclick=orderform() value='   提交   '><input class=hfdiv id=con name=con>"
setTimeout(function(){iF.scrollIntoView(true);G("til").focus()},500)

function orderform(){
if(checksuv&&checksuv()||checkesuv&&checkesuv()||checkatt&&checkatt())return
if(vT<2&&til.value.length<1){alert("发帖需要标题！");til.focus();return}
var E=Zz=Is=Y=O=0
...
MF.method="POST"
MF.action="sup.php?type="+vT+"&actionid="+sID+"&tid="+tID
MF.submit()
}
...
cvLan={}
Os={}
function loadC(c,f){var o;A(D.body,o=C("iframe"));o.className='ifcode';o.o=c;o[IE?'onreadystatechange':'onload']=f;o.src="js/lan/"+c.charCodeAt(0)+".htm"}
...
function(){return doexe("FormatBlock",false,"<"+(this==this.parentNode.childNodes[1]?"p":this.lastChild.innerHTML)+">",this)})
shortm(M.H,this)
}
...
function getAtts(){if(!M.A)M.A=fillwin(M.A,0,IE?426:436,"e/"+(IE?"ie":"fx")+"att.htm");centerWin(M.A,2)}

function getLink(){if(!M.L)M.L=fillwin(M.L,400,300,"e/link.htm");centerWin(M.L,0)}

function getCode(){if(!M.D)M.D=fillwin(M.D,500,400,"e/code.htm");centerWin(M.D,0)}

function html(){if(!M.M)M.M=fillwin(M.M,0,IE?426:436,"e/html.htm");else HTML();centerWin(M.M,2)}
...
function dosuv(){if(!M.V)M.V=fillwin(M.V,IE?500:510,30,"e/suv.htm",MF);centerWin(M.V,0)}

function about(){if(!M.B)M.B=fillwin(M.B,400,198,"e/about.htm");centerWin(M.B,0)}
...
if(!IE){I=[1,2,3,4,14,15,16,17,18,19,20];for(i=0;i<I.length;)mns[I[i++]]=0}
